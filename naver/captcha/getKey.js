/* 
Express는 웹 모바일 애플리케이션[하이브리드앱(웹+앱)]을 위한 일련의 강력한 기능을 제공하는 간결하고
유연한 Node.js 웹 애플리케이션 프레임워크입니다.
ExpressAPI를 이용하면 자유롭게 호라용할 수 있는 수많은 HTTP 유틸리티 메소드 및 미들웨어를 통해
쉽고 빠르게 강력한 API를 작성할 수 있습니다
 */
const express = require("express"); //웹 어플리케이션을 위한 프레임워크 선언
// 선언하면 프레임워크가 제공하는 API를 누릴 수 있다. 그럴때 nodejs에서는 require함수를 사용함
// 리액트에서는 import가 제공됨
// <script type="module"></script> 지원하게 되면서 넥플릭스가 서버사이드를 노드제이에스기반
const app = express();
// 네이버 서버에서 발급한 고유아이디값 - 회원가입이 되어 있고 개발자센터에서 어플리케이션을 등록한자
const client_id = "B81fDOZrQsT6oRUETnq3"; //개발자센터에서 발급받은 Client ID
// 개발자센터에서 어플리케이션을 등록하고 받는 비번
const client_secret = "OdaWI0I7fx"; //개발자센터에서 발급받은 Client Secret
const code = "0";
let key;
...
// 익스프레스도 그래서 제공하고 있다.
// req는 요청객체의 인스턴스변수로 express로 부터 객체 주입 받는다. - 사용자가 생성 할 수 없다.
